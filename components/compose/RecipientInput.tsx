...
export function RecipientInput({ value, onChange, placeholder = "Add recipients..." }: RecipientInputProps) {
    ...
    <div className="flex flex-wrap items-center gap-1 min-h-[36px] px-2 py-1 border border-input rounded-md bg-background focus-within:ring-2 focus-within:ring-ring">
      {value.map((recipient, i) => (
        <span key={i} className={cn(
          ...
          isValidEmail(recipient.address) ? "bg-secondary text-secondary-foreground" : "bg-destructive/20 text-destructive"
        )}>
          <span className="font-mono">{recipient.name || recipient.address}</span>
          <button onClick={() => onChange(value.filter((_, idx) => idx !== i))} className="hover:text-foreground">
