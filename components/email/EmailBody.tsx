...
export function EmailBody({ email, thread = [] }: EmailBodyProps) {
  ...
  const { markRead, markStarred, removeEmail, setEmails, emails } = useEmailStore()
  ...
  const handleReplyAll = () => openCompose({
  ...
  const processedBody = showImages
    ? email.body
    : email.body.replace(/src="(https?:\/\/[^"]+)"/g, 'data-blocked-src="$1" src=""')
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <span className="text-xs text-muted-foreground ml-2 font-mono">&lt;{email.from.address}&gt;</span>
            </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <Button variant="ghost" size="icon" className="w-8 h-8" onClick={() => void handleArchive()} title="Archive">
          <Archive className="w-4 h-4" />
        </Button>
        <Button variant="ghost" size="icon" className="w-8 h-8" onClick={() => void handleTrash()} title="Delete">
      ...
      {!showImages && /src="https?:\/\//.test(email.body) ? (
        <div className="flex items-center justify-between px-3 py-2 rounded-md bg-muted/50 border border-border text-xs">
          <span className="text-muted-foreground">External images are blocked</span>
          <button onClick={() => setShowImages(true)} className="text-primary hover:underline">
            Load images
          </button>
        </div>
      ) : null}
      ...
      <div className="prose prose-invert prose-sm max-w-none">
        {email.body ? (
          ...
          <pre className="whitespace-pre-wrap text-sm font-sans">{email.preview}</pre>
        )}
      </div>
