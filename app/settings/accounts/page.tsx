        ...
        {accounts.map((account) => (
          ...
          >
            <div
              ...
              style={{
                backgroundColor:
                  (account.color as string) ||
                  PROVIDER_COLORS[account.provider as string] ||
                  "#6366F1",
              }}
            >
              {((account.email ?? account.providerAccountId) as string)
                .charAt(0)
                .toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-medium text-sm truncate">
                {(account.displayName || account.email) as string}
              </div>
              <div className="text-xs text-muted-foreground">
                {account.email as string} ·{" "}
                <span className="capitalize">{account.provider as string}</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span
                className={`text-xs px-2 py-1 rounded-full ${account.isActive ? "bg-green-500/10 text-green-400" : "bg-muted text-muted-foreground"}`}
              >
